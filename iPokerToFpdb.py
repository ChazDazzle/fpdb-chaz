...
class iPoker(HandHistoryConverter):
    ...
    re_PlayerInfo = re.compile(
        r'<player( (seat="(?P<SEAT>[0-9]+)"|name="%(PLYR)s"|chips="(%(LS)s)?(?P<CASH>[%(NUM)s]+)(%(LS)s)?"|dealer="(?P<BUTTONPOS>(0|1))"|win="(%(LS)s)?(?P<WIN>[%(NUM)s]+)(%(LS)s)?"|bet="(%(LS)s)?(?P<BET>[^"]+)(%(LS)s)?"|addon="\d*"|rebuy="\d*"|merge="\d*"|reg_code="[\d-]*"))+\s*/>' % substitutions,
        re.MULTILINE)
    re_Board = re.compile(r'<cards( (type="(?P<STREET>Flop|Turn|River)"|player=""))+>(?P<CARDS>.+?)</cards>',
                          re.MULTILINE)
    ...
    re_HeroCards = re.compile(
        r'<cards( (type="(Pocket|Second\sStreet|Third\sStreet|Fourth\sStreet|Fifth\sStreet|Sixth\sStreet|River)"|player="%(PLYR)s"))+>(?P<CARDS>.+?)</cards>' % substitutions,
        re.MULTILINE)
    re_Action = re.compile(
        r'<action ((no="(?P<ACT>[0-9]+)"|player="%(PLYR)s"|(actiontxt="[^"]+" turntime="[^"]+")|type="(?P<ATYPE>\d+)"|sum="(%(LS)s)?(?P<BET>[%(NUM)s]+)(%(LS)s)?"|cards="[^"]*") ?)*/>' % substitutions,
        re.MULTILINE)
        ...
        if tourney:
                ...
                if mg['BIRAKE']:
                    ...
                    if m4:
                        ...
                        self.tinfo['buyin'] = int(
                            100 * Decimal(self.clearMoneyString(self.re_non_decimal.sub('', mg['BIAMT']))))
                        if 'BIRAKE1' in mg and mg['BIRAKE1']:
                            self.tinfo['buyin'] += int(
                                100 * Decimal(self.clearMoneyString(self.re_non_decimal.sub('', mg['BIRAKE1']))))
            ...
            if self.info['limitType'] == 'fl' and self.info['bb'] is not None:
                try:
                    ...
                    tmp = handText[0:200]
                    log.error(
                        _("iPokerToFpdb.determineGameType: Lim_Blinds has no lookup for '%s' - '%s'") % (mg['BB'], tmp))
                    raise FpdbParseError
    ...
    def readHandInfo(self, hand):
        ...
        if m2:
            ...
            m2.group('Y'), month, m2.group('D'), m2.group('H'), m2.group('MIN'), sec)
            ...
            for found in m:
                ...
                if player is not None:
                    ...
                    if street == 'THIRD' and len(newcards) == 3 and self.hero == player:  # hero in stud game
                        ...
                        hand.addHoleCards(street, player, closed=newcards[0:2], open=[newcards[2]], shown=True,
                                          mucked=False, dealt=False)
                    elif street == 'SECOND' and len(newcards) == 2 and self.hero == player:  # hero in stud game
                        hand.hero = player
                        hand.dealt.add(player)
                        hand.addHoleCards(street, player, closed=[newcards[0]], open=[newcards[1]], shown=True,
                                          mucked=False, dealt=False)
                    else:
                        hand.addHoleCards(street, player, open=newcards, closed=oldcards, shown=True, mucked=False,
                                          dealt=False)
    ...
    def readAction(self, hand, street):
        ...
        for a in sorted(actions.iterkeys()):
            ...
            if atype == '0':
                ...
                log.error(_("DEBUG:") + " " + _("Unimplemented %s: '%s' '%s'") % (
                "readAction", action['PNAME'], action['ATYPE']))
    ...
    def getTableTitleRe(type, table_name=None, tournament=None, table_number=None):
        log.info("iPoker getTableTitleRe: table_name='%s' tournament='%s' table_number='%s'" % (
